...
export const BgIntroduction = () => {
  ...
  useEffect(() => {
    if (homePageData && homePageData[0]?.background) {
      setIsLoading(false);
    }
  }, [homePageData]);
  ...
  const handleArrowClick = direction => {
    ...
    const newIndex =
      direction === 'left'
        ? indexIntroduction === 0
          ? homePageData[0]?.background?.length - 1
          : indexIntroduction - 1
        : indexIntroduction === homePageData[0]?.background?.length - 1
          ? 0
          : indexIntroduction + 1;
    setIndexIntroduction(newIndex);
  };

  const backgroundImageUrl =
    !isLoading &&
    ImageURL + homePageData[0]?.background[indexIntroduction]?.background;
  return (
    <section
      className={
    ...
    >
      {isLoading ? (
        ...
        <>
          ...
          >
            {homePageData[0]?.background?.map((bg, index) => (
              <div
                className={`w-[30px] sm:w-[104px] h-1 rounded-sm  ${
                  indexIntroduction === index
